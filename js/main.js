...
(function () {
  ...
  /* ------------------------------------------------------------ timing */
  var TM = {
    /* paced like the reference: splash 0–1.2 s, logo reveal 1.2–2.8 s, loading from ~3 s */
    titleStart: .9, titleSweepEnd: 2.1, dissStart: 2.5, dissEnd: 3.1, panelIn: 2.6, loadStart: 3.0
  };
  var k;
  ...
  /* everything after loading hits 100 %: the lily bursts at once, the player is in by ~1 s */
  var OUT = { pulse: .3, dissStart: 0, dissEnd: .75, morph: .1, titleIn: .45, titleDur: 1.1, autoplay: 1.2, finalStart: .1, finalEnd: 1.4, kill: .7 };
  ...
  var KF = {
    /* darkness over the scene (1 = black). The scene opens as deep burgundy and lifts as the haze spreads. */
    dark:     [[0, .72], [.9, .68], [2.0, .16], [L0 + .8, .18], [L0 + 1.8, .30], [F, .20], [F + .5, .10], [F + 1.4, 0]],
    reach:    [[0, .10], [.9, .18], [2.4, 1]],                     // how far the haze has spread from the centre
    heat:     [[.3, 0], [1.6, .3], [2.8, .5], [F - 1, .55], [F, .6], [F + 1.4, 1]],   // coral-red → pink → (finally) the original white
    atmos:    [[0, .10], [1.2, .14], [2.4, .12], [L0, .06], [F, .03], [F + .8, 0]],
    vignette: [[0, .30], [1.6, .24], [2.8, .14], [F + .6, .10], [F + 1.4, 0]],
    wisp:     [[0, 0], [1.0, .05], [2.0, .4], [3.0, .9], [L0 + 1.6, .5], [F, .55], [F + 1.2, 0]],
    titleGlow:[[.6, 0], [1.6, .20], [2.5, .22], [3.1, .08], [F + .5, .05], [F + 1.4, 0]],
    trailS:   [[0, .3], [.4, .95], [1.4, .9], [2.0, .12], [2.4, 0]],        // opening swirl fades away for the logo reveal
    trailL:   [[L0 - .2, 0], [L0 + .6, 1], [F, 1], [F + .9, 0]],        // strands converging on the node while loading
    dust:     [[0, .5], [1.0, .95], [2.8, .85], [F, .55], [F + .6, 0]],
    micro:    [[0, .25], [1.2, .5], [2.8, .7], [F, .8], [F + 1.4, .5]],
    glitter:  [[.2, .4], [1.2, .8], [2.4, .8], [F + 1.4, .85]],
    star:     [[.2, .6], [1.2, .9], [2.8, 1], [L0, .8], [F + 1.4, .3]],
    dens:     [[0, 1], [F + .4, 1], [F + 1.6, .72]],
    leaf:     [[L0 + .3, 0], [L0 + 1.4, .6], [F, .65], [F + .7, 0]]
  };
  ...
  var time = 0, titleHold = 0, stage = '', flags = { flare1: T > .5, flare2: T > 1.5, flare3: T > F + 1.3, node: T > L0 + .4, autoplay: T > F + OUT.autoplay };
  ...
  function titleAt(t) {
    ...
    else reveal(t, TF, OUT.titleDur, 6, .55);
    return TS;
  }
  ...
  function render(dt, rawDt) {
    ...
    /* ---- title ---- */
    var tone = T < TM.dissEnd ? 'coral' : 'white';
    ...
    if (!reduced) {
      /* the big starbursts of the opening sit on the light ribbons, like in the reference */
      if (!flags.flare1 && T >= 0) { flags.flare1 = true; fx.flare(.63, .17, 118, 3.4, .8); fx.flare(.57, .545, 84, 3.6, .9); fx.flare(.81, .745, 104, 3.8, 1.0); fx.flare(.30, .262, 46, 3.0, .7); fx.flare(.17, .655, 40, 3.2, .8); }
      if (!flags.flare2 && T > 1.5) { flags.flare2 = true; fx.flare(.32, .25, 58, 3.4); fx.flare(.77, .43, 52, 3.2); fx.flare(.16, .72, 48, 3.4); }
    ...
    P.leafA = reduced ? 0 : kf(T, KF.leaf); P.dens = kf(T, KF.dens);
    ...
    P.starCap = Math.round(lerp(15, 6, smooth(F, F + 1.4, T)));
    P.starScale = lerp(1, .5, smooth(F + .3, F + 1.4, T));               // the last frame has small, soft sparkles only
  ...
  window.CL.debug = { get T() { return T; }, timeline: { L0: L0, F: F, TF: TF }, buildMs: function () { return fx.buildMs(); }, level: function () { return fx.level(); }, frameMs: function () { return fxMs; } };
})();
