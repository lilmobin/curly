...
(function () {
  ...
  var KF = {
    /* darkness over the scene (1 = black). The haze is present from the very first frame and expands from the centre. */
    dark:     [[0, .95], [1.6, .88], [2.8, .66], [4.8, .40], [L0, .20], [F, .10], [F + .5, .10], [F + 1.4, 0]],
    reach:    [[0, .10], [5.6, 1]],                     // how far the pink atmosphere has expanded
    heat:     [[1.6, 0], [5.2, 1]],                     // crimson → white for lines, dust and sparkles
    atmos:    [[0, .05], [1.2, .09], [2.4, .14], [4.0, .18], [L0, .10], [F, .03], [F + .8, 0]],
    vignette: [[0, 0], [2.5, .10], [L0, .16], [F + .6, .10], [F + 1.4, 0]],
    wisp:     [[0, .03], [2.2, .05], [3.2, .16], [4.6, .34], [L0 + .6, .52], [F, .52], [F + 1.2, 0]],
    titleGlow:[[1.8, 0], [3.2, .20], [4.8, .22], [L0, .08], [F + .5, .05], [F + 1.4, 0]],
    trail:    [[.9, 0], [2, .55], [4, .85], [L0, .8], [F, .75], [F + .9, 0]],
    micro:    [[0, 0], [1.5, 0], [2.4, .4], [4, .8], [F + 1.4, .5]],
    glitter:  [[1.6, 0], [2.6, .55], [3.8, .9], [F + 1.4, .85]],
    star:     [[1.4, 0], [2.2, .5], [4, .9], [L0, .8], [F + 1.4, .3]],
    dens:     [[0, 1], [F + .4, 1], [F + 1.6, .72]],
    leaf:     [[L0 + .5, 0], [F, .6], [F + .7, 0]]
  };
  ...
  var el = {
    ...
    titleFx: $('titleFx'), card: $('card'), loadingUI: $('loadingUI'), loadTrack: $('loadTrack'), loadFill: $('loadFill'), loadDot: $('loadDot')
  };
  ...
  var fx = window.CL.createFX(el.canvas, {
    maxDPR: CFG.maxDPR || 1.5, reduced: reduced,
  ...
  var time = 0, titleHold = 0, stage = '', flags = { flare1: T > 1.15, flare2: T > 3.9, flare3: T > F + 1.3, autoplay: T > F + OUT.autoplay };
  ...
  function render(dt, rawDt) {
    ...
    if (!idle || (frameN & 1) === 0) { P.dt = Math.min(fxAcc, .066); P.rawDt = fxRaw; P.idle = idle; fx.frame(P); fxAcc = 0; }
    player.tick();
  }

  var last = 0, frameN = 0, fxAcc = 0, fxRaw = 0;
  function loop(now) {
  ...
  window.CL.debug = { get T() { return T; }, timeline: { L0: L0, F: F, TF: TF }, buildMs: function () { return fx.buildMs(); }, level: function () { return fx.level(); } };
})();
