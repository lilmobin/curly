...
(function () {
  ...
  var KF = {
    ...
    wisp:     [[0, 0], [2.2, 0], [3.2, .16], [4.6, .34], [L0 + .6, .52], [F, .52], [F + 1.6, .40], [F + 4, .16]],
    titleGlow:[[1.8, 0], [3.2, .20], [4.8, .22], [L0, .08], [F + .9, .05], [F + 2.4, .04]],
    trail:    [[.9, 0], [2, .55], [4, .85], [L0, .8], [F, .75], [F + 2.4, .45], [F + 6, .30]],
    micro:    [[0, 0], [1.5, 0], [2.4, .4], [4, .8], [F + 2.6, .55]],
    glitter:  [[1.6, 0], [2.6, .55], [3.8, .9], [F + 2.6, .55]],
    star:     [[1.4, 0], [2.2, .5], [4, .9], [L0, .8], [F + 2.6, .5], [F + 6, .35]],
    dens:     [[0, 1], [F + .5, 1], [F + 4.5, .55]],
    leaf:     [[L0 + .5, 0], [F, .6], [F + 1.4, .7], [F + 3, .42], [F + 6, .3]]
  };
  ...
  function layout() {
    ...
    /* proportions follow the reference: a wider, airier glass card sitting a little below the middle */
    var cardW = Math.min(Math.max(.80 * S, 280), vw - 28), cardH = clamp(.575 * cardW, 150, 186);
    var loadW = Math.min(Math.max(.75 * S, 250), cardW), loadH = 124 * u;
    var gap = .13 * S, usable = vh - safeTop - safeBot;
    var cardCY = safeTop + usable * .55;
    var titleCY = Math.max(cardCY - cardH / 2 - gap - titleH / 2, safeTop + titleH / 2 + 10);
    var introCY = safeTop + usable * .455;                      // the reference reveals the logo near the middle of the screen

    var st = root.style;
    st.setProperty('--u', u.toFixed(3));
    st.setProperty('--title-w', titleW.toFixed(1) + 'px'); st.setProperty('--title-cy', introCY.toFixed(1) + 'px');
    ...
    G = { vw: vw, vh: vh, cx: vw / 2, titleCY: titleCY, introCY: introCY, curCY: introCY, titleW: titleW, titleH: titleH, cardCY: cardCY };
    fx.resize(vw, vh, {
      S: S,
      flower: { bx: vw / 2, by: cardCY - cardH / 2 + .2 * cardH, L: .42 * S },
      title: { cx: vw / 2, cy: introCY, w: titleW, h: titleH },
  ...
  function render(dt, rawDt) {
    ...
    /* ---- title: revealed near the middle (as in the reference), settles above the player ---- */
    var wantCY = T < TM.dissEnd ? G.introCY : G.titleCY;
    if (wantCY !== G.curCY) { G.curCY = wantCY; document.documentElement.style.setProperty('--title-cy', wantCY.toFixed(1) + 'px'); fx.setTitle(G.cx, wantCY); }
    if (!titleMissing) {
    ...
    if (!reduced && !titleMissing && ou > 0 && ou < 1) {
      ...
      var oy = G.curCY + Math.sin(r * TAU * 1.6) * .13 * G.titleH - .03 * G.titleH;
    ...
    var RES = (CFG.residualFlower === undefined ? .4 : CFG.residualFlower);
    P.residual = reduced ? 0 : kf(T, [[F + 2.4, RES], [F + 7.5, RES * .2]]);   // faint lily stays through 'home', then settles into the clean idle scene
  ...
  window.CL.debug = { get T() { return T; }, timeline: { L0: L0, F: F, TF: TF }, buildMs: function () { return fx.buildMs(); } };
})();
