...
(function (global) {
  ...
  function createFX(canvas, opts) {
    ...
    function renderTrails(p) {
      ...
      updateTrails(p);
      for (i = 0; i < trails.length; i++) { tr = trails[i]; if (tr.ga > .01 && tr.head > 0) { any = true; if (tr.head < 1.28) moving = true; } }
      trailMoving = moving;
    ...
    function ensureGrade() {
      ...
      if (!grade || gw !== w || gh !== h) { grade = makeCanvas(w, h); gctx = grade.getContext('2d'); gw = w; gh = h; }
    }
    ...
    function draw(p) {
      ...
      if (p.finalMix >= .999) {
        ...
        var aOut = 1 - (1 - lerp(.86, p.dark, smooth(.7, 1, p.reach))) * p.fadeIn;   // edges start deep burgundy (not black), like the reference
