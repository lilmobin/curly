...
(function (global) {
  ...
  var NODE = [.62, .665];
  var TRAIL_DEFS = [
    { id: 'A',  grp: 's', t0: .3, t1: 3.4, amp: 1,   pts: [[-.06, .235], [.18, .19], [.42, .168], [.63, .17], [.84, .19], [1.06, .23]] },
    { id: 'A2', grp: 's', t0: .7, t1: 3.8, amp: .6,  thin: true, spk: 7, pts: [[-.06, .40], [.10, .33], [.30, .265], [.52, .215], [.63, .178]] },
    { id: 'B',  grp: 's', t0: .4, t1: 3.9, amp: 1,   pts: [[1.06, .43], [.86, .50], [.55, .555], [.25, .60], [.05, .655], [.10, .72], [.38, .76], [.68, .77], [.86, .75], [1.06, .79]] },
    { id: 'B2', grp: 's', t0: .9, t1: 4.3, amp: .55, thin: true, spk: 8, pts: [[1.06, .445], [.86, .515], [.55, .57], [.25, .615], [.055, .668], [.115, .733], [.38, .773], [.68, .783], [.86, .765], [1.06, .805]] },
    { id: 'B3', grp: 's', t0: 1.3, t1: 4.6, amp: .5,  thin: true, spk: 6, pts: [[1.06, .84], [.85, .80], [.60, .82], [.30, .87], [-.05, .92]] },
  ...
  function createFX(canvas, opts) {
    ...
    /* -------------------------------------------------------- particles */
    var MICRO_MAX = 320, GLITTER_MAX = 110;
    var micro = [], glitter = [], stars = [], tsparks = [], leaves = [];
    var dust = [], DUST_MAX = 700;
      ...
      for (i = 0; i < trails.length; i++) {
        ...
        if (tr.ga > .01 && tr.head > 0) {
          ...
          if (tr.head > .02 && tr.head < 1.05 && !reduced) {      // particles travel with the head of each line
            ...
            for (var pc = 0; pc < 3; pc++) spawnFree(tr.samples[2 * hi2] + rnd(-4, 4), tr.samples[2 * hi2 + 1] + rnd(-4, 4), rnd(-12, 12), rnd(-16, 4), rnd(1.8, 3.6), rnd(1.6, 3.2), rnd(.7, 1), .5, 0);
          }
        }
      }
      trailMoving = moving;
    ...
    function drawParticles(p) {
      ...
      for (i = 0; i < n; i++) {
        ...
        if (p.lightCap < .999) { a *= hzAt(px, py, p.T); if (a < .01) continue; }
    ...
    function drawStars(p) {
      ...
      for (i = 0; i < stars.length; i++) {
        ...
        if (p.lightCap < .999) { env *= hzAt(px, py, p.T); if (env < .02) continue; }
    ...
    /* ---------------------------------------------------------- draw all */
    /* ---- HAZE THAT THE LINES CARRY --------------------------------------------------------------
       Every point of every opening line has a known arrival time (when its head passes). From there the
       haze spreads outward at a fixed speed, so each pixel of the screen has an "arrival time" A(x,y).
       A pixel stays black until the haze reaches it, then thickens smoothly. Nothing brightens by itself:
       particles and stars use the same field, so they also only appear where the haze has arrived.      */
    var lightA = null, lctxA = null, hzImg = null, arr = null, nearW = null, noiseM = null, coverT = 6;
    var HZ_SPEED = 95, HZ_RAMP = 1.5;
    function invEase(y) { var lo = 0, hi = 1, m, k; for (k = 0; k < 22; k++) { m = (lo + hi) / 2; if (easeInOutCubic(m) < y) lo = m; else hi = m; } return (lo + hi) / 2; }
    function buildArrival() {
      arr = new Float32Array(gw * gh); nearW = new Float32Array(gw * gh); noiseM = new Float32Array(gw * gh);
      var k = gw / W, sc = S / 390, v = HZ_SPEED * sc * k, lam = 70 * sc * k, spacing = Math.max(1.2, gw / 95), srcs = [], i, j, q, def, pts, tmp, cnt, sm, u, t, dx, dy, dd, tt, best, near, idx, cx, cy, mx = 0;
      for (i = 0; i < TRAIL_DEFS.length; i++) {
        def = TRAIL_DEFS[i]; if (def.grp !== 's') continue;
        pts = []; for (j = 0; j < def.pts.length; j++) pts.push([def.pts[j][0] * W, def.pts[j][1] * H]);
        tmp = new Float32Array(((pts.length - 1) * SPS + 1) * 2); cnt = catmull(pts, tmp);
        var prev = null;
        for (j = 0; j < cnt; j++) {                         // arrival time of the line's head at this sample
          sm = j / (cnt - 1); u = invEase(Math.min(.999, sm / 1.28)); t = def.t0 + u * (def.t1 - def.t0);
          var cxs = tmp[2 * j] * k, cys = tmp[2 * j + 1] * k;
          if (prev) {                                       // fill in points so the sources are evenly dense (no comb pattern in the haze)
            var seg = Math.sqrt((cxs - prev[0]) * (cxs - prev[0]) + (cys - prev[1]) * (cys - prev[1])), m = Math.max(1, Math.ceil(seg / spacing)), z;
            if (seg < spacing * .5 && j < cnt - 1) continue;         // already dense enough here → skip (keeps the build cheap on phones)
            for (z = 1; z < m; z++) { var f = z / m; srcs.push(prev[0] + (cxs - prev[0]) * f, prev[1] + (cys - prev[1]) * f, prev[2] + (t - prev[2]) * f); }
          }
          srcs.push(cxs, cys, t); prev = [cxs, cys, t];
        }
      }
      var ns = srcs.length / 3;
      for (j = 0; j < gh; j++) for (i = 0; i < gw; i++) {
        cx = i + .5; cy = j + .5; best = 1e9; near = 1e9;
        for (q = 0; q < ns; q++) {
          dx = cx - srcs[3 * q]; dy = cy - srcs[3 * q + 1]; dd = Math.sqrt(dx * dx + dy * dy); tt = srcs[3 * q + 2] + dd / v;
          if (tt < best) best = tt; if (dd < near) near = dd;
        }
        idx = j * gw + i; arr[idx] = best; nearW[idx] = Math.exp(-near / lam); noiseM[idx] = fbm(i / gw * 3.2 + 2.1, j / gh * 5.4 + 7.3);
        if (best > mx) mx = best;
      }
      coverT = mx + HZ_RAMP;
    }
    function ensureGrade() {
      ...
      if (!grade || gw !== w || gh !== h) {
        ...
        lightA = makeCanvas(w, h); lctxA = lightA.getContext('2d'); hzImg = lctxA.createImageData(w, h);
        for (var i = 0; i < w * h; i++) { hzImg.data[4 * i] = 255; hzImg.data[4 * i + 1] = 255; hzImg.data[4 * i + 2] = 255; }
        if (!NT) initNoise();
        buildArrival();
      }
    }
    function hzAt(px, py, T) {                              // 0 before the haze arrives, 1 once it has settled
      if (!arr) return 1;
      var i = Math.min(gw - 1, Math.max(0, (px / W * gw) | 0)), j = Math.min(gh - 1, Math.max(0, (py / H * gh) | 0)), t = (T - arr[j * gw + i]) / 1.1;
      return t <= 0 ? 0 : (t >= 1 ? 1 : t * t * (3 - 2 * t));
    }
    function updateHaze(p) {
      ensureGrade();
      var d = hzImg.data, T = p.T, cap = p.lightCap, base = (1 - p.dark) * p.fadeIn, cs = smooth(.6, 1, cap), i, n = gw * gh, t, a, m;
      for (i = 0; i < n; i++) {
        t = (T - arr[i]) / HZ_RAMP;
        if (t <= 0) { d[4 * i + 3] = 0; continue; }
        a = t >= 1 ? 1 : t * t * (3 - 2 * t);
        m = (.42 + .58 * nearW[i]) * (.72 + .56 * noiseM[i]); m += (1 - m) * cs;        // denser near the lines and patchy, until it has spread evenly
        a = a * m * cap * base; d[4 * i + 3] = a >= 1 ? 255 : (a * 255) | 0;
      }
      lctxA.putImageData(hzImg, 0, 0);
    }
    ...
    function draw(p) {
      ...
      var reachR = (.08 + 1.1 * p.reach) * R, dr = reduced ? .1 : 1, hzOn = !reduced && p.lightCap < .999;
      ...
      if (p.finalMix >= .999) {
        ...
        /* darkness + vignette + final field are composed on a tiny canvas and blitted once.
           While the haze is spreading the darkness is carved away only where the haze has arrived. */
        var aIn = 1 - (1 - p.dark) * p.fadeIn, hazeMode = !reduced && p.lightCap < .999;
        if (hazeMode || aIn > .002 || p.vignette > .01 || p.finalMix > .002) {
          ensureGrade();
          gctx.setTransform(gw / W, 0, 0, gh / H, 0, 0); gctx.clearRect(0, 0, W, H); gctx.globalAlpha = 1;
          if (hazeMode) {
            updateHaze(p);
            gctx.fillStyle = '#000'; gctx.fillRect(0, 0, W, H);
            gctx.save(); gctx.setTransform(1, 0, 0, 1, 0, 0); gctx.globalCompositeOperation = 'destination-out'; gctx.drawImage(lightA, 0, 0, gw, gh); gctx.restore();
          } else if (aIn > .002) {
            gctx.fillStyle = 'rgba(0,0,0,' + aIn.toFixed(3) + ')'; gctx.fillRect(0, 0, W, H);
          }
          if (p.vignette > .01) {
    ...
    return {
      ...
      arrivalAt: function (nx, ny) { if (!arr) return 0; var i = Math.min(gw - 1, Math.max(0, (nx * gw) | 0)), j = Math.min(gh - 1, Math.max(0, (ny * gh) | 0)); return arr[j * gw + i]; },
      coverTime: function () { return coverT; },
